...
const keywordValidator = (keyword, flags) => {
  ...
  if (keyword && !blacklist.includes(keyword)) {
    return true;
  }
...
const getLoggerPreferncePrompt = () => {
  const typePromptGenerator = getPromptGenerator('type', 'list', 'Choose the logger preference to update:');
  typePromptGenerator.addProperty('choices', ['maxsize', 'blacklist']);
  return typePromptGenerator.getPrompt();
};

...
const changeSessionDuration = (time) => {
  if (!time) {
    return prefPrompts.changeSessionDurationPrompt();
  }
...
const changeTimeout = async (options) => {
  const { time } = options;
  let { timeoutType: type } = options;
  if (!type) {
    const typePrompt = prefPrompts.getTimeoutsTypePrompt();
    ({ type } = (await inquirer.prompt(typePrompt)));
  }
  if (type === 'session') {
    return changeSessionDuration(time);
  }

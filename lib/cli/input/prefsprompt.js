...
const getTimeoutsTypePrompt = () => {
  const typePromptGenerator = getTypePromptGenerator('type', 'list', 'Choose the timeout type:', undefined, ['session']);
  return typePromptGenerator.getPrompt();
};
...
const changeSessionDurationPrompt = async () => {
  const sessionDurationPromptGenerator = getPromptGenerator('time', 'input', 'Enter session timeout:');
  const { time } = await inquirer.prompt(sessionDurationPromptGenerator.getPrompt());

  return {
    name: 'timeouts_changed',
    details: {
      time,
      type: 'session',
    },
...
module.exports = {
  ...
  getServerTypePrompt,
  changeLoggerPrompt,
  changeSessionDurationPrompt,
  getTimeoutsTypePrompt,
};

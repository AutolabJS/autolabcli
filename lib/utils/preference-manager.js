...
const deleteCredentials = () => {
  ...
  logger.moduleLog('debug', 'Preference Manager', 'Crednetials deleted.');
};
...
const setDefaultPreferences = () => {
  logger.moduleLog('debug', 'Preference Manager', 'Setting Default Preferences.');
  if (!cliPrefs.gitlab) {
...
const storeGitlabPrefs = (values) => {
  ...
  logger.moduleLog('debug', 'Preference Manager', 'Gitlab preferences updated.');
  logger.moduleLog('debug', 'Preference Manager', values);
...
const setPreference = (options) => {
  switch (options.name) {
    ...
    case 'cliPrefs':
      ...
      logger.moduleLog('debug', 'Preference Manager', 'CLIPrefs updated.');
      logger.moduleLog('debug', 'Preference Manager', options.values);
      return;
    default:

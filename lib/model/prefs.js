...
const storeSessionTimeout = (time) => {
  preferenceManager.setPreference(getPrefsObjectToStore(
    'cliPrefs',
    {
      timeouts: {
...
const storeTimeoutsPref = ({ type, time }) => {
  if (type === 'session') {
    storeSessionTimeout(time);
  }
};
...
const storePrefs = (changePrefEvent) => {
  ...
  } else if (changePrefEvent.name === 'timeouts_changed') {
    storeTimeoutsPref(changePrefEvent.details);
  }
